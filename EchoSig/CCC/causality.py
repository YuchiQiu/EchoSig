...
def CCC_grangercausalitytests(x, maxlag,causal_stc=['tf',['ligand','receptor']], addconst=True, verbose=None):
    """
    ...
    causal_stc: list
        ...
        2. It can contain three variables. e.g., ['tf',['ligand','receptor']]. 
           It examines if the pair in second element granger causes the first element.
               i.e., does ligand-recpetor pair: L(t-lag)*R(t) causes TF(t)
               The time lag only consider for ligand.
                -- This is a modified granger causality for Ligand-Receptor-TF pairs.
...
def compute_lagged_values(target,ligand,lag_list,receptor=None,metrics = mean_squared_error):
    ...
    Returns:
        stat (np.array): A 1D NumPy array representing statistical values of the L-R-TF signal under different lags.
    """
    if receptor is None:
...
def find_lag_regulate(target,ligand,lag_list,receptor=None,metrics='rmse',curve_thred=0.2,p_thred=0.01):
...
def evaluate_causality(target,ligand,lag_list,max_lag,dt,receptor=None,metrics='rmse',
                       max_target=1.,max_ligand=1.,max_receptor=1.,curve_thred=0.2,p_thred=0.01,
                       time_unit='h'):
    """_summary_
...
def trajCCC(source_id,target_id,z_traj,time,lag_list,max_lag,
    ...
    """ Calculate CCC causality between two cell trajectories for given L-R-TF info
    ...
    Args:
        ...
        df_LRTF (pd.DataFrame): 
            A DataFrame containing prior knowledge of L-R-TF interactions
        ...
        p_thred (float, optional): thredhold for p_value for statistical significance
            Default to 0.01
        save_fig (bool, optional): Whether output L-R-TF time series curves (only show inferred interactions). 
            Defaults to False.
    ...
    Returns:
        pd.DataFrame: A DataFrame, each row is a L-R-TF pair, and its CCC causality is given in the DataFrame.
    """
    ...
    data = {}
    for itm in df_LRTF.iterrows():
        ligand_name = itm[1]['Ligand']
        # The prior-knowledge table still uses its legacy ``TF`` input column.
        SPG_name = itm[1]['TF']
