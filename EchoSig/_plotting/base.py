...
class PlotterPriorGraph:
    ## draw L-R-SPG graph with edges from prior knowledge graph, 
    # and use causality results to highlight signficiant edges.
    def __init__(self, CCC_data,test='ftest'):
        # self.L_list=L_list
        # self.R_list=R_list
        # self.RSPG_list=RSPG_list
        self.CCC_data=CCC_data
        key = list(CCC_data.index)
    ...
    def create_G(self,test='ftest'):
        edge_nonsig = []
        G = nx.DiGraph()
        G.add_nodes_from(self.L_list,bipartite=0)
        G.add_nodes_from(self.RSPG_list,bipartite=1)

        for index in self.CCC_data.index:
            itm=self.CCC_data.loc[index]
            ligand,receptor,SPG = index.split('-')
    #    for itm in result_granger.iterrows():
            # ligand,receptor,SPG = itm[0].split('-')
            u=ligand
            v=receptor+'-'+SPG
            if u in G.nodes and v in G.nodes:
                ...
                if q <0.05 :
                    ...
                    edge_nonsig.append((u,v))
        self.G=G
        self.edge_nonsig=edge_nonsig
        # return G, edge_nonsig
    def G_layout(self,layout='circle',
                        inner_radius=1,outer_radius=6,time_unit='h',time_scale=1):
        if layout=='horizontal':
            pos = nx.multipartite_layout(self.G,
                                subset_key='bipartite',
                                align='horizontal')
            ...
            pos = circular_bipartite_layout(self.G,self.L_list,self.RSPG_list,inner_radius=inner_radius,outer_radius=outer_radius)
        labels = {node: (node.split('-')[1] 
                        if node in self.RSPG_list else node) 
                        for node in self.G.nodes()}
        
        # unique_keys = sorted(set(node.split('-')[0] for node in RSPG_list))
        color_palette = plt.cm.get_cmap("tab10", len(self.R_list)+1)  # Get distinct colors
        color_map = {}
        for ligand in self.L_list:
            color_map[ligand]=color_palette(0)
        for idx, key in enumerate(self.R_list):
            color_map[key] = color_palette(idx+1)  # Assign a color
        node_colors = []
        for node in self.G.nodes():
            if node in self.RSPG_list:
                receptor = node.split('-')[0]
                node_colors.append(color_map[receptor])
            else:
                node_colors.append(color_map[node])  # ligand color
        edge_widths = [self.G[u][v].get("stat") for u, v in self.G.edges()]
        edge_lag = {(u,v): f"{time_scale*self.G[u][v].get('SPT'):.0f}{time_unit}" for u, v in self.G.edges()}
        ...
        self.edge_attr = {'width':edge_widths,
                          'SPT':edge_lag}

    def draw_L_RSPG_G(self,
               layout='circle',inner_radius=1,outer_radius=6,
               time_unit='h',time_scale=1,edge_width_scale=1.,node_font_size=8, 
               show_legend=True, title=None,ax=None,
               save_fig=None):

            #    G,edge_nonsig,pos, labels,edge_widths,edge_lag,node_colors, color_palette,
            #         width_scale=1.,
            #        
            #         
            #         ):
        
        self.G_layout(layout,inner_radius=inner_radius,outer_radius=outer_radius,time_unit=time_unit,time_scale=time_scale)
        pos = self.node_attr['pos']
        labels=self.node_attr['label']
        node_colors=self.node_attr['color']
        color_palette=self.node_attr['color_palette']
        edge_widths=self.edge_attr['width']
        edge_lag=self.edge_attr['SPT']
        if ax is None:
            ax = plt.gca()
        nx.draw_networkx_nodes(self.G,pos=pos,node_color=node_colors,
                            ax=ax
                            )
        # positive regulation
        if (edge_widths>0).sum()>0:
            nx.draw_networkx_edges(self.G,pos=pos,
                                width=np.maximum(edge_widths, 0)*edge_width_scale,
                                ax=ax
                                )
        # negative regulation
        if (edge_widths<0).sum()>0:
            nx.draw_networkx_edges(self.G,pos=pos,
                                width=-np.minimum(edge_widths,0)*edge_width_scale,
                                arrowstyle='-[',
                                ax=ax
                                )
        # nx.draw_networkx_edges(self.G,pos=pos,edgelist=self.edge_nonsig,arrows=False,
        #                     edge_color='gray',style='dashed',alpha=0.4,width=0.4,
        #                     ax=ax
        #                     )
        nx.draw_networkx_labels(self.G,pos=pos,labels = labels,font_size=node_font_size,
                                ax=ax)
        nx.draw_networkx_edge_labels(self.G,pos,edge_labels=edge_lag,
                                    ax=ax)
        if title is not None:
            ax.set_title(title)
        if show_legend:
            legend_patches = [mpatches.Patch(color=color_palette(idx), label=key+' (R)') if key!='ligands' 
                            else mpatches.Patch(color=color_palette(idx), label=key)
                            for idx, key in enumerate(['ligands']+self.R_list)]
            solid_line = mlines.Line2D([], [], color='black', linestyle='solid', label='q<0.05')
            dashed_line = mlines.Line2D([], [], color='gray', linestyle='dashed', label='n.s.')    
            ax.legend(handles=legend_patches+[solid_line,dashed_line], 
                    loc="upper left",           
...
class BaseGraph:
    ...
    def create_G(self,**kwargs):
        ...
        # self.node_colors={}
        # num_L=len(self.L_all)
        # num_R=len(self.R_all)
        # num_SPG=len(self.SPG_all)
    ...
    def create_legend(self,palette_mode='gene',node_colors=None,**kwargs):
        if node_colors is None: 
            if palette_mode=='gene':
                ...
                legend_patches = []
                for cell_id in self.cell_list:
                    if not self.palette.get(cell_id):
                        continue
