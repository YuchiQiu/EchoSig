...
class Circle_L_R_SPG_L(BaseCircleGraph):
    ## UNWEIGHTED graph for L_source-R-SPG-L_target between multiple cells
    def __init__(self,test='ftest',net=['L','R','SPG','L']):
        super().__init__(test)
        self.net=net
    ...
    def create_edges(self,time_scale=1,time_unit='h',**kwargs):
        self.edge_LR={}
        self.edge_RSPG={}
        for key in self.cell_pairs:
            ...
            for i in range(len(LR_avg)):
                ...
                self.edge_LR[source_cell+' L '+u,target_cell+' R ' + v] = LR_avg.iloc[i]['SPT']
            for i in range(len(RSPG_avg)):
                ...
                self.edge_RSPG[target_cell+' R '+v,target_cell+' SPG '+w] = RSPG_avg.iloc[i]['stat '+self.test]
        ...
        for key in self.GRN.keys():
            ...
            for idx_grn in range(len(self.GRN[key])):
                ...
                self.edge_SPGT[cell+' SPG '+itm['source'],cell + ' L '+itm['target']] =itm['score']
    ...
    def draw_G(self,ax=None,palette_mode='cell_gene',node_colors=None,**kwargs):
        """_summary_
        ...
        if ax is None:
            ax = plt.gca()
        # nx.draw_networkx_nodes(self.G,pos=pos,node_color=[self.node_colors.get(node) for node in self.G.nodes])
        # nx.draw_networkx_labels(self.G,pos,self.node_label)
        nx.draw_networkx_edges(self.G,self.pos,#edge_labels=self.edge_LR_lag,
        ...
        nx.draw_networkx_edges(self.G,self.pos,#edge_labels=self.edge_LR_lag,
                                     ...
                                     ax=ax,
                                     width=3,alpha=0.6)
        nx.draw_networkx_edge_labels(self.G,self.pos,edge_labels=self.edge_RSPG_positive,
                                    ax=ax,font_size=7,alpha=0.7)
        nx.draw_networkx_edges(self.G,self.pos,#edge_labels=self.edge_LR_lag,
                               arrowstyle='-[',
                                     ...
                                     ax=ax,
                                     width=3,alpha=0.6)
        nx.draw_networkx_edge_labels(self.G,self.pos,edge_labels=self.edge_RSPG_negative,
                                    ax=ax,font_size=7,alpha=0.7)
        ...
        nx.draw_networkx_edges(self.G,self.pos,#edge_labels=self.edge_LR_lag,
                                     ...
                                     ax=ax,
                                     width=3,alpha=0.6)
        nx.draw_networkx_edge_labels(self.G,self.pos,edge_labels=self.edge_SPGT_positive,
        ...
        nx.draw_networkx_edges(self.G,self.pos,#edge_labels=self.edge_LR_lag,
                               arrowstyle='-[',
                                     ...
                                     ax=ax,
                                     width=3,alpha=0.6)
        nx.draw_networkx_edge_labels(self.G,self.pos,edge_labels=self.edge_SPGT_negative,
                                    ax=ax,font_size=6,alpha=0.6)
        self.draw_nodes()
        plt.box(False)
        plt.axis('equal')
...
class Circle_L_SPG_L(BaseCircleGraph):
    ...
    def create_edges(self,time_scale=1,time_unit='h',abs_LRSPG=False):
        ...
        for key in self.GRN.keys():
            ...
            for idx_grn in range(len(self.GRN[key])):
                itm = self.GRN[key][idx_grn]
                self.edge_SPGT[cell+' SPG '+itm['source'],cell + ' '+signal_net[0]+' '+itm['target']] =itm['score']
    ...
    def draw_G(self,palette_mode='cell_gene',node_colors=None,width_scale=1.,ax=None,**kwargs):
        ...
        nx.draw_networkx_edges(self.G,self.pos,#edge_labels=self.edge_LR_lag,
                                     edgelist=list(self.edge_SPGT_positive.keys()),
                                     edge_color=[self.node_colors.get(u) for (u,v) in list(self.edge_SPGT_positive.keys())],
                                     width = [width_scale*self.edge_SPGT_positive[(u, v)] for (u, v) in list(self.edge_SPGT_positive.keys())],
                                     ax=ax,
                                     alpha=0.7)
        ...
        nx.draw_networkx_edges(self.G,self.pos,#edge_labels=self.edge_LR_lag,
                               arrowstyle='-[',
                                     edgelist=list(self.edge_SPGT_negative.keys()),
                                     edge_color=[self.node_colors.get(u) for (u,v) in list(self.edge_SPGT_negative.keys())],
                                     width = [-width_scale*self.edge_SPGT_negative[(u, v)] for (u, v) in list(self.edge_SPGT_negative.keys())],
                                     ax=ax,
                                     alpha=0.7)
    ...
    def save_graph(self,save_dir):
        ...
        for (u,v) in self.edge_LRSPG.keys():
            ...
            data1.append({
                ...
                'SPT': self.edge_LRSPG[(u,v)],
            })
        data2=[]
        for (u,v) in self.edge_SPGT.keys():
            source_cell, source_type,source_gene = u.split(' ')
            target_cell, target_type,target_gene = v.split(' ')
            data2.append({
                'source': u,
...
class Circle_L_L(Circle_L_SPG_L):
    ...
    #     super().create_G(**kwargs)
    #     ## prune nodes that are not in edges
    #     ## only this plot may have nodes without edges. 
    ...
    def draw_G(self,ax=None,palette_mode='cell',node_colors=None,width_scale=0.1,width_thred=0.01,node_size=150,
        ...
        # nx.draw_networkx_edges(self.G,self.pos,
        #                              edgelist=list(self.edge_LL_positive.keys()),
        #                              edge_color=[self.node_colors.get(u) for (u,v) in list(self.edge_LL_positive.keys())],
        #                              ax=ax,
        #                              width=[width_scale*self.edge_LL_positive[(u, v)] for (u, v) in list(self.edge_LL_positive.keys())],
        #                              alpha=0.7)
        # nx.draw_networkx_edges(self.G,self.pos,#edge_labels=self.edge_LR_lag,
        #                        arrowstyle='-[',
        #                              edgelist=list(self.edge_LL_negative.keys()),
    ...
    # def draw_G(self,ax=None,palette_mode='cell',width_scale=0.1,**kwargs):
    #     """_summary_
    ...
    #     self.node_layout(palette_mode=palette_mode,**kwargs)        
    ...
    def create_edges(self,time_scale=1,time_unit='h',abs_LRSPG=False):
        ...
        for key in self.cell_pairs:
            ...
            for i in range(len(signal_lag)):
                itm=signal_lag.iloc[i]
                u=itm[self.signal_net[0]]
                self.L_lag[key][u] = time_scale*itm['SPT']        
        ...
        #             strength = self.edge_LRSPG[u] * self.edge_SPGT[v]
        #             self.edge_LL[node_pair] += strength
        # self.edge_LL_positive = {node:self.edge_LL.get(node) for node in list(self.edge_LL.keys()) if self.edge_LL.get(node)>0}
        # self.edge_LL_negative = {node:self.edge_LL.get(node) for node in list(self.edge_LL.keys()) if self.edge_LL.get(node)<0}
