...
# Define a function to save the models' states
def save_checkpoint(epoch, func, optimizer, path, **kwargs):
    state = {
        'epoch': epoch,
...
class UOT(nn.Module):
    ...
    def forward(self, t, states):
        ...
        # Divergence is no longer computed in the current (x, logm) system,
        # so gradients with respect to the state do not need to be forced on.
    ...
    def trajectory(self,num_sample, t_eval,time_idx,data_train,sigma,device='gpu'):
        ...
        # x.requires_grad_(True)  # only needed for divergence/Jacobian
        logm_diff_t1 = torch.zeros(x.shape[0], 1).type(torch.float32).to(device)
        # g_t1 = logp_diff_t1
        # z, _, _ = odesolve(self,y0=(x, g_t1, logp_diff_t1),options=options)
        z, _= odeint(self,y0=(x, logm_diff_t1),
...
def trace_df_dz(f, z):
    ...
    for i in range(z.shape[1]):
        # The trace itself is part of the ODE loss graph. Keep the velocity
        # network graph available so loss.backward() can differentiate through
        # this Jacobian after the forward ODE solve has completed.
        diagonal_grad = torch.autograd.grad(
            f[:, i].sum(),
            z,
            create_graph=True,
            retain_graph=True,
        )[0]
        sum_diag += diagonal_grad.contiguous()[:, i].contiguous()
