...
def get_v(model_ae,func,z_flatten,time_flatten,dt,device='cuda'):
    ...
    vel_flatten = vel_net(time_flatten, z_flatten).detach()
    ...
    if model_ae is not None:
        ...
        z_original_flatten=None
        vel_original_flatten=None
    vel_flatten = vel_flatten.cpu().numpy()
...
def get_trajectory(num_sample,time,time_GRN,data_train,model_ae,func,sigma,device,dt=None,time_ascend=True,batch_size=1000):
    ...
    # z_original_flatten2,vel_flatten2,vel_original_flatten2 = get_v(model_ae,func,z_flatten,time_flatten,dt,device=device)
    # # J_vx_x_flatten = get_J(model_ae,func,z_flatten,time_flatten,dt,device=device)
    # g_flatten2 = get_g(model_ae,func,z_flatten,time_flatten,dt)
    # # dg_flatten = get_dg(model_ae,func,z_flatten,time_flatten,dt,device=device)
